...
{
	ObjectName& obj_name = program.nameTable[nameIndex];
	...
	case ObjectType::Constant: new_obj.set(new int(*(int*)obj_name.namePointer), ValueType::Integer); break;
	case ObjectType::LongConstant: new_obj.set(new LongNat(*(LongNat*)obj_name.namePointer), ValueType::LongNat); break;
	}
...
void Interpreter::compare(CmpValue operation)
{
	if (stack.size() < 2)
	{
		error();
...
void Interpreter::execute_token(Token token)
{
